...
// Creates a pending row in skylar_pairing_requests for the bot to claim.
  ...
  try {
    ...
    if (premiumMode) {
      ...
      const { rows: krows } = await query(
        'SELECT id FROM skylar_premium_keys WHERE key = $1 AND status = $2 LIMIT 1',
        [key, 'unused']
      );
      if (!krows[0]) return json(res, 403, { error: 'Invalid or already used activation key.' });
      await query('UPDATE skylar_premium_keys SET status = $1, used_phone = $2, used_at = now() WHERE id = $3',
        ['used', phone, krows[0].id]);
    }

    // ── Insert pairing request ────────────────────────────
    const { rows } = await query(
      `INSERT INTO skylar_pairing_requests (phone, status) VALUES ($1, 'pending') RETURNING id, phone, expires_at`,
      [phone]
    );
